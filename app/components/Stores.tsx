                ...
                {stores?.map((store) => {
                    return (
                        <tr key={store.id}>
                            ...
                            <td className="border-b-2 border-e-2 border-emerald-400 py-2 gap-2 px-3 ">
...
const STORE_TYPE_WRITE = async ({ stores }: { stores?: IStoreModel[] }) => {
    ...
    return (
        <table className="table-auto border-separate w-full text-center border-spacing-0">
            <thead>
                <tr>
                    <th scope="col" className=" w-1/4 border-l-[3px] border-y-[3px] border-emerald-400 bg-emerald-600 rounded-tl-2xl">STORE ONCE</th>
                    <th scope="col" className=" w-1/4 border-[3px] border-emerald-400 bg-emerald-600">IP</th>
                    <th scope="col" className=" w-1/4 border-[3px] border-emerald-400 bg-emerald-600">Modelo</th>
                    <th scope="col" className=" w-1/4 border-r-[3px] border-y-[3px] border-emerald-400 bg-emerald-600 rounded-tr-2xl">Acciones</th>
                </tr>
            </thead>
            <tbody>
                {stores?.map((store) => {
